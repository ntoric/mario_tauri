...
const TopSellingCategoriesReport: React.FC = () => {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      // Export the file
      const success = await saveCSVWithDialog(csv, `top-selling-categories-${dateRange}.csv`);
                ...
                <table className="zoho-table">
                  ...
                  <tbody>
                    {pagination.paginatedItems(sorted).map((row, idx) => (
                      <tr key={`${row.categoryId}-${row.date}`} style={{ background: idx % 2 === 0 ? undefined : 'var(--gray-50)' }}>
                        <td style={{ fontWeight: 600, color: 'var(--dark)' }}>{row.category}</td>
                        <td>{row.date}</td>
                  ...
                  <tfoot>
                    <tr style={{ borderTop: '2px solid var(--gray-300)', background: 'var(--gray-100)' }}>
                      <td colSpan={2} style={{ padding: '10px 14px', fontWeight: 800, color: 'var(--dark)' }}>Total</td>
